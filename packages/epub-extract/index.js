...
function epubExtract(srcFile, options = {}) {
    ...
    return epub2_1.EPub.createAsync(srcFile)
        ...
        let path_novel = path_1.default.join(PATH_NOVEL_MAIN, text_1.fixText(fs_iconv_2.trimFilename(epub.metadata.title)));
        let currentVolume;
        let volume_list = [];
        let lastLevel = 0;
        ...
        await bluebird_1.default.mapSeries(volume_list, async function (volume) {
            ...
            let dirname = path_1.default.join(path_novel, `${vid} ${fs_iconv_2.trimFilename(volume.volume_title)}`);
            return bluebird_1.default.mapSeries(volume.chapter_list, async function (chapter) {
                let ext = '.txt';
                // @ts-ignore
                let name = fs_iconv_2.trimFilename(chapter.chapter_title);
...
function getCheerio(doc) {
    let $ = cheerio_1.default.load(html_1.fixHtml2(doc));
    cheerio_2.fixCheerio('body', $);
    return $;
}

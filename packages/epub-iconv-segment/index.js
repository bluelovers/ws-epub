...
async function stringifySegment(input) {
    return (0, novel_segment_cli_1.stringify)(await (0, novel_segment_cli_1.textSegment)(input));
}
function handleGlobSegment(pattern, options) {
    return (0, lib_1.handleGlob)(pattern, {
        ...options,
        iconvFn: {
            async tw(input) {
                return stringifySegment(input).then(v => (0, min_1.cn2tw_min)(v, {
                    safe: false
                }));
            },
            async cn(input) {
                return stringifySegment(input).then(v => (0, min_1.tw2cn_min)(v));
            },
        }
    });
}
